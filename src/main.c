...
typedef struct {
  ...
  gboolean   connected;
  gboolean   suppress;   /* skip auto-apply during programmatic updates */

  /* current colour as HSV (each 0..1) */
  double     hue, sat, val;
...
{
  /* 0 → 20000 ms (slow), 100 → 1000 ms (fast) */
...
{
  guint sel = gtk_drop_down_get_selected(GTK_DROP_DOWN(w->mode_drop));
  /* 0 static, 1 breathing, 2 cycle, 3 wave */
